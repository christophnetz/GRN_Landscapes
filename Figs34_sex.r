...
noise_df2 <- expand.grid(x = x_coords, y = y_coords)
noise_df2$z <- as.vector(noise_matrix)
...
dplyr::filter(processed_results2, n > 10)
...
data <- dplyr::select(final_results_complex, motif, rep, x1, x2, peak_reached2)
...
extract_pixel_borders <- function(landscape_data, res = 200) {
  ...
  pred <- predict(svm_model, landscape_data)
  print(mean(pred == landscape_data$peak_reached))
...
master_border_df2 <- do.call(rbind, all_borders_list)


ggplot(noise_df2, aes(x = x, y = y)) +
  ...
  geom_tile(data = dplyr::select(dplyr::filter(master_border_df2, motif == 6), -motif),
            aes(x = x1, y = x2),
            fill = "red") +
  new_scale_fill() +
  geom_point(
  ...
  geom_tile(data = master_border_df2, aes(x = x1, y = x2), fill = "white") +
...
###
# Simplified fig
# facet_grid over frequency
# take motifs 1, 2 and 6

load("~/fig34_data.RData")

noise_df2$freq <- 0.1
final_results2$freq <- 0.1
processed_results2$freq <- 0.1
master_border_df2$freq <- 0.1

final_results_simple <- final_results2
processed_results_simple <- processed_results2
master_border_df_simple <- master_border_df2
noise_df_simple <- noise_df2

final_results2$freq <- 0.5
processed_results2$freq <- 0.5

master_border_df2 <- master_border_df
master_border_df2$freq <- 0.5
noise_df2 <- noise_df2
noise_df2$freq <- 0.5

final_results_complex <- final_results2
processed_results_complex <- processed_results2
master_border_df_complex <- master_border_df2
noise_df_complex <- noise_df2

noise_df_all <- rbind(noise_df_simple, noise_df_complex)
master_border_df_all <- rbind(master_border_df_simple, master_border_df_complex)
final_results_all <- rbind(final_results_simple, final_results_complex)
processed_results_all <- rbind(processed_results_simple, processed_results_complex)


plot34 <- ggplot(noise_df_all, aes(x = x, y = y)) +
  ...
  # --- Overlay: domain borders only (discrete fill)
  geom_segment(
    data = filter(final_results_all, motif %in% c(1,2)),
    aes(
      x = x1,
      y = x2,
      xend = x1_final,
  ...
  geom_point(
    data = filter(processed_results_all, motif %in% c(1,2)),
    aes(x = x1, y = x2, color = n),
    size = 0.2,
    inherit.aes = FALSE
  ) +  geom_tile(data = dplyr::select(dplyr::filter(master_border_df_all, motif == 6), -motif),
                 aes(x = x1, y = x2),
                 fill = "red",
  ...
  new_scale_fill() +
  scale_colour_gradient(low = "white", high = "black") +
  geom_tile(data = filter(master_border_df_all, motif %in% c(1,2)), aes(x = x1, y = x2), fill = "white",
  ...
  #  clip = "on"
  #) +
  facet_grid( freq~ motif, labeller = labeller(
    motif = function(x)
      paste("Motif", x), 
    freq = function(x)
      paste("Landscape complexity", x)
  )) +
...
saveRDS(plot34, "my_plot.rds")
my_plot <- readRDS("my_plot.rds")
ggsave("fig3_new_poster.png", height = 8, width = 13)
ggsave("fig3_new.png", height = 8, width = 13)
